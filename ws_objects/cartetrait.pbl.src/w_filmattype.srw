$PBExportHeader$w_filmattype.srw
forward
global type w_filmattype from w_child
end type
type cb_print from commandbutton within w_filmattype
end type
type st_title from statictext within w_filmattype
end type
type cb_delete from commandbutton within w_filmattype
end type
type cb_update from commandbutton within w_filmattype
end type
type dw_filmattype from u_dw within w_filmattype
end type
type cb_close from commandbutton within w_filmattype
end type
type cb_insert from commandbutton within w_filmattype
end type
end forward
...
cb_delete cb_delete
cb_update cb_update
dw_filmattype dw_filmattype
cb_close cb_close
cb_insert cb_insert
end type
global w_filmattype w_filmattype
...
public subroutine sauve ();if dw_filmattype.Update() = 1 then
	commit using SQLCA;
	dw_filmattype.retrieve(v_no_ortho)
	change = false
else
...
this.dw_filmattype=create dw_filmattype
...
this.Control[iCurrent+5]=this.dw_filmattype
this.Control[iCurrent+6]=this.cb_close
this.Control[iCurrent+7]=this.cb_insert
end on

on w_filmattype.destroy
call super::destroy
destroy(this.cb_print)
destroy(this.st_title)
destroy(this.cb_delete)
destroy(this.cb_update)
destroy(this.dw_filmattype)
...
string text = "Type de matériaux pour fil"
boolean focusrectangle = false
end type

event constructor;if v_langue = 'an' then
	this.Text = "Material type for wire"
end if
end event
...
ll_idraptrait = dw_filmattype.getitemnumber(dw_filmattype.getrow(),'id_raptrait')
...
if ll_nb > 0 then
	...
	if messagebox("Suppression","Voulez-vous supprimer l'enregistrement?",Question!,YesNo!) =1 then
		dw_filmattype.deleterow(dw_filmattype.getrow())
		change = true
	end if
end if
end event
...
string dataobject = "d_filmattype"
boolean border = false
boolean ib_isupdateable = false
...
ll_newrow = dw_filmattype.InsertRow(0)
dw_filmattype.ScrollToRow(ll_newrow)
dw_filmattype.setitem(ll_newrow,'ortho_id',1)
dw_filmattype.SetFocus()
end event
