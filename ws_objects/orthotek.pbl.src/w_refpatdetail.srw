$PBExportHeader$w_refpatdetail.srw
forward
global type w_refpatdetail from w_child
end type
type cb_retrieve from commandbutton within w_refpatdetail
end type
type uo_calutil1 from u_calutil within w_refpatdetail
end type
type uo_calutil2 from u_calutil within w_refpatdetail
end type
type em_fin from editmask within w_refpatdetail
end type
type st_2 from statictext within w_refpatdetail
end type
type em_debut from editmask within w_refpatdetail
end type
type st_1 from statictext within w_refpatdetail
end type
type dw_refpatdet from u_dw within w_refpatdetail
end type
type cb_close from commandbutton within w_refpatdetail
end type
type cb_print from commandbutton within w_refpatdetail
end type
end forward
...
em_debut em_debut
st_1 st_1
dw_refpatdet dw_refpatdet
cb_close cb_close
cb_print cb_print
end type
global w_refpatdetail w_refpatdetail

on w_refpatdetail.create
int iCurrent
call super::create
...
this.dw_refpatdet=create dw_refpatdet
...
long ll_horidspec, li_liste_trait_par_spec

//dw_refpatdet.object.t_from.text = em_debut.text
//dw_refpatdet.object.t_to.text = em_fin.text
//ldt_start = date(em_debut.text)
//ldt_end = date(em_fin.text)

em_debut.getdata(ldt_start)
em_fin.getdata(ldt_end)
...
if li_liste_trait_par_spec = 1 then
	dw_refpatdet.retrieve(v_no_ortho,ldt_start,ldt_end,v_no_ortho)
else
	if isnull(ll_horidspec) then ll_horidspec = v_no_ortho
	dw_refpatdet.retrieve(v_no_ortho,ldt_start,ldt_end,ll_horidspec)
end if
end event
...
integer height = 1900
integer taborder = 10
string title = "refpatdet"
string dataobject = "d_refpatdet"
...
event constructor;if v_langue = 'an' then
	this.object.t_title.text = 'Referring dentist detail report'
...
openwithparm(w_print_options,dw_refpatdet)
end event
