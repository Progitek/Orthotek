$PBExportHeader$w_referents_patient.srw
forward
global type w_referents_patient from w_child
end type
type uo_calutil2 from u_calutil within w_referents_patient
end type
type uo_calutil1 from u_calutil within w_referents_patient
end type
type cb_find from commandbutton within w_referents_patient
end type
type st_1 from statictext within w_referents_patient
end type
type em_debut from editmask within w_referents_patient
end type
type em_fin from editmask within w_referents_patient
end type
type st_2 from statictext within w_referents_patient
end type
type dw_referents_patient from u_dw within w_referents_patient
end type
type cb_close from commandbutton within w_referents_patient
end type
type cb_print from commandbutton within w_referents_patient
end type
end forward
...
em_fin em_fin
st_2 st_2
dw_referents_patient dw_referents_patient
cb_close cb_close
cb_print cb_print
end type
global w_referents_patient w_referents_patient
...
this.dw_referents_patient=create dw_referents_patient
...
this.Control[iCurrent+8]=this.dw_referents_patient
this.Control[iCurrent+9]=this.cb_close
this.Control[iCurrent+10]=this.cb_print
end on

on w_referents_patient.destroy
call super::destroy
...
destroy(this.em_fin)
destroy(this.st_2)
destroy(this.dw_referents_patient)
...
dw_referents_patient.retrieve(il_refid,ldt_start,ldt_end)
arret = 0
end event
...
event clicked;dw_referents_patient.retrieve(il_refid,date(em_debut.text),date(em_fin.text))
...
event constructor;long ll_year
string ls_date

ll_year = year(today()) - 2
em_debut.text = '01-01-' + string(ll_year)
end event
...
integer height = 1892
integer taborder = 60
string title = "referents_patient"
string dataobject = "d_referents_patient"
...
if v_langue = 'an' then
	...
	this.object.t_date.text = 'Traitment Date'
...
openwithparm(w_print_options,dw_referents_patient)
