$PBExportHeader$w_etatrdv.srw
forward
global type w_etatrdv from w_child
end type
type ddlb_typetrait from u_ddlb within w_etatrdv
end type
type uo_calutil2 from u_calutil within w_etatrdv
end type
type uo_calutil1 from u_calutil within w_etatrdv
end type
type cb_print from commandbutton within w_etatrdv
end type
type em_fin from editmask within w_etatrdv
end type
type st_2 from statictext within w_etatrdv
end type
type em_debut from editmask within w_etatrdv
end type
type st_1 from statictext within w_etatrdv
end type
type cb_find from commandbutton within w_etatrdv
end type
type dw_statpatcons from u_dw within w_etatrdv
end type
type cb_close from commandbutton within w_etatrdv
end type
end forward
...
int iCurrent
call super::create
this.ddlb_typetrait=create ddlb_typetrait
...
this.Control[iCurrent+1]=this.ddlb_typetrait
...
on w_etatrdv.destroy
call super::destroy
destroy(this.ddlb_typetrait)
...
event selectionchanged;call super::selectionchanged;il_indtrait = index
end event

event constructor;call super::constructor;string ls_nomtrait
long ll_idtrait, i = 1

DECLARE listtypetrait CURSOR FOR
...
FETCH listtypetrait INTO :ll_idtrait, :ls_nomtrait;

DO WHILE SQLCA.SQLCode = 0
	
	if isnull(ls_nomtrait) = false then
		addItem(ls_nomtrait)
		il_traitid[i] = ll_idtrait
		i++
	end if
...
dw_statpatcons.retrieve(v_no_ortho,debut,fin,il_traitid[il_indtrait])
end event
