...
type cb_2 from commandbutton within w_password
end type
type cb_1 from commandbutton within w_password
end type
...
type sle_anc from singlelineedit within w_password
end type
type sle_nouveau from singlelineedit within w_password
end type
type sle_verif from singlelineedit within w_password
end type
end forward
...
if sle_anc.text <> "" and sle_nouveau.text <> "" and sle_verif.text = sle_nouveau.text then
	select password into :v_password from password where id = 1;
	if v_password = sle_anc.text then
		v_password = sle_nouveau.text
		update password set password = :v_password where id = 1;
		commit;
		close(parent)
	else
