$PBExportHeader$w_typeradio.srw
forward
global type w_typeradio from w_child
end type
type cb_print from commandbutton within w_typeradio
end type
type st_title from statictext within w_typeradio
end type
type cb_delete from commandbutton within w_typeradio
end type
type cb_update from commandbutton within w_typeradio
end type
type dw_typeradio from u_dw within w_typeradio
end type
type cb_close from commandbutton within w_typeradio
end type
type cb_insert from commandbutton within w_typeradio
end type
end forward
...
cb_delete cb_delete
cb_update cb_update
dw_typeradio dw_typeradio
cb_close cb_close
cb_insert cb_insert
end type
global w_typeradio w_typeradio
...
public subroutine sauve ();if dw_typeradio.Update() = 1 then
	commit using SQLCA;
	dw_typeradio.retrieve(v_no_ortho)
	change = false
else
...
this.dw_typeradio=create dw_typeradio
...
this.Control[iCurrent+5]=this.dw_typeradio
this.Control[iCurrent+6]=this.cb_close
this.Control[iCurrent+7]=this.cb_insert
end on

on w_typeradio.destroy
call super::destroy
destroy(this.cb_print)
destroy(this.st_title)
destroy(this.cb_delete)
destroy(this.cb_update)
destroy(this.dw_typeradio)
...
string text = " Types de radiographies"
boolean focusrectangle = false
end type

event constructor;if v_langue = 'an' then
	this.Text = "Radiography type"
end if
end event
...
ll_idraptrait = dw_typeradio.getitemnumber(dw_typeradio.getrow(),'id_raptrait')
...
if ll_nb > 0 then
	...
	if messagebox("Suppression","Voulez-vous supprimer l'enregistrement?",Question!,YesNo!) =1 then
		dw_typeradio.deleterow(dw_typeradio.getrow())
		change = true
	end if
end if
end event
...
integer height = 1840
integer taborder = 10
string title = "types de radiographie"
string dataobject = "d_typeradio"
boolean border = false
boolean ib_isupdateable = false
...
ll_newrow = dw_typeradio.InsertRow(0)
dw_typeradio.ScrollToRow(ll_newrow)
dw_typeradio.setitem(ll_newrow,'ortho_id',1)
dw_typeradio.SetFocus()
end event
