...
public function integer of_sendsms (string as_numero, string as_sujet, string as_msg)
end prototypes
...
public function integer of_sendsms (string as_numero, string as_sujet, string as_msg);string ls_smtpserver, ls_smtpuserid, ls_smtppassword, ls_body, ls_filename, ls_file[], ls_tab[], ls_smtpsender
string ls_from
long ll_smtpauth, ll_logfile, i, ll_iduser
integer li_smtpport, li_rc, li_max, li_idx
...
select 	isnull(smtpserver,''), isnull(smtpport,25), isnull(smtpuserid,''), isnull(smtppassword,''), 
			isnull(smtpauth,0), isnull(logfile,0)
into		:ls_smtpserver, :li_smtpport, :ls_smtpuserid, :ls_smtppassword, :ll_smtpauth, :ll_logfile
from t_options
where ortho_id = :v_no_ortho;

select isnull(courriel,'') into :ls_from from t_personnels where id_personnel = :gl_idpers;

if ls_smtpsender = "" then 

	select isnull(prenom,'') || ' ' || isnull(nom,'') into :ls_smtpsender 
	from t_personnels
...
//ls_body +="<body><PRE>"+as_msg+"</PRE></body>"
//ls_body +="</html>"

ls_body = as_msg

pbsmtpmail.SenderName = ls_smtpsender
pbsmtpmail.SenderEmail = ls_from
pbsmtpmail.SmtpServer = ls_smtpserver
pbsmtpmail.SmtpPort = li_smtpport

if ll_smtpauth = 1 then
	pbsmtpmail.SmtpAuth = true
	pbsmtpmail.SmtpUser = ls_smtpuserid
	pbsmtpmail.SmtpPassword = ls_smtppassword
end if

ls_tab[1] = as_numero + "@txt.bell.ca"
ls_tab[2] = as_numero + "@vmobile.ca"
ls_tab[3] = as_numero + "@msg.telus.com"
ls_tab[4] = as_numero + "@pcs.rogers.com"
ls_tab[5] = as_numero + "@fido.ca"
ls_tab[6] = as_numero + "@msg.koodomobile.com"
ls_tab[7] = as_numero + "@mobiletxt.ca"

for i = 1 to upperbound(ls_tab)
	pbsmtpmail.ajoutRecipient(ls_tab[i])
next
pbsmtpmail.messageText = ls_body
...
IF li_rc <> 1 THEN 
	MessageBox ("Avertissement", "Erreur: " + String ( li_rc ) )
END IF
