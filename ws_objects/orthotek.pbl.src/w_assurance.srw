$PBExportHeader$w_assurance.srw
forward
global type w_assurance from w_child
end type
type cb_3 from commandbutton within w_assurance
end type
type cb_signature from commandbutton within w_assurance
end type
type cb_2 from commandbutton within w_assurance
end type
type st_patnom from statictext within w_assurance
end type
type cb_add from commandbutton within w_assurance
end type
type pb_4 from picturebutton within w_assurance
end type
type pb_3 from picturebutton within w_assurance
end type
type pb_2 from picturebutton within w_assurance
end type
type pb_1 from picturebutton within w_assurance
end type
type st_title from statictext within w_assurance
end type
type cb_1 from commandbutton within w_assurance
end type
type cb_delete from commandbutton within w_assurance
end type
type cb_print from commandbutton within w_assurance
end type
type cb_transfert from commandbutton within w_assurance
end type
type cb_update from commandbutton within w_assurance
end type
type cb_close from commandbutton within w_assurance
end type
type dw_assurance from u_dw within w_assurance
end type
end forward
...
global w_assurance w_assurance
...
on w_assurance.create
int iCurrent
call super::create
this.cb_3=create cb_3
this.cb_signature=create cb_signature
...
this.Control[iCurrent+2]=this.cb_signature
this.Control[iCurrent+3]=this.cb_2
this.Control[iCurrent+4]=this.st_patnom
...
this.Control[iCurrent+14]=this.cb_transfert
this.Control[iCurrent+15]=this.cb_update
this.Control[iCurrent+16]=this.cb_close
this.Control[iCurrent+17]=this.dw_assurance
end on

on w_assurance.destroy
call super::destroy
destroy(this.cb_3)
destroy(this.cb_signature)
destroy(this.cb_2)
...
luo_sign.of_affichesign( dw_assurance, 2, dw_assurance.getitemnumber(1, 'assurance_id'))
end event
...
if change then 
	...
	if ll_nbrow > 0 then
		...
		luo_sign.of_affichesign(ds_ass, 2, ll_idass)
		...
		if isvalid = false and isvalid2 = false then
			if isnull(ds_ass.getItemDate(ds_ass.getRow(),'assurance_formulaire_datetrait')) then
				...
				if isnull(ds_ass.getItemDate(ds_ass.getRow(),'date_paiement_initial')) then
...
long ll_x, ll_y, ll_assid
n_signature luo_sign

ll_x = UnitsToPixels(integer(dw_assurance.object.p_sign.width), XUnitsToPixels!)
ll_y = UnitsToPixels(integer(dw_assurance.object.p_sign.height), YUnitsToPixels!)
...
if ls_sign <> "" then
	...
	luo_sign.of_setsignature(ls_sign, 2, ll_assid)	
end if
...
if change then 
	...
	// Afficher la signature
	luo_sign.of_affichesign(ds_estime, 2, ll_idass)
