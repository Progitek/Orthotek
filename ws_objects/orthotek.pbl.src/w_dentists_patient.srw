$PBExportHeader$w_dentists_patient.srw
forward
global type w_dentists_patient from w_child
end type
type uo_calutil2 from u_calutil within w_dentists_patient
end type
type uo_calutil1 from u_calutil within w_dentists_patient
end type
type cb_find from commandbutton within w_dentists_patient
end type
type st_1 from statictext within w_dentists_patient
end type
type em_debut from editmask within w_dentists_patient
end type
type em_fin from editmask within w_dentists_patient
end type
type st_2 from statictext within w_dentists_patient
end type
type dw_dentists_patient from u_dw within w_dentists_patient
end type
type cb_close from commandbutton within w_dentists_patient
end type
type cb_print from commandbutton within w_dentists_patient
end type
end forward
...
em_fin em_fin
st_2 st_2
dw_dentists_patient dw_dentists_patient
cb_close cb_close
cb_print cb_print
end type
global w_dentists_patient w_dentists_patient

type variables
private long il_detid
int arret

end variables

on w_dentists_patient.create
int iCurrent
call super::create
...
this.dw_dentists_patient=create dw_dentists_patient
...
this.Control[iCurrent+8]=this.dw_dentists_patient
this.Control[iCurrent+9]=this.cb_close
this.Control[iCurrent+10]=this.cb_print
end on

on w_dentists_patient.destroy
call super::destroy
...
destroy(this.em_fin)
destroy(this.st_2)
destroy(this.dw_dentists_patient)
...
il_detid = message.doubleparm //parmetre externe
dw_dentists_patient.retrieve(il_detid,ldt_start,ldt_end)
arret = 0
end event
...
event clicked;dw_dentists_patient.retrieve(il_detid,date(em_debut.text),date(em_fin.text))
...
integer height = 1892
integer taborder = 60
string title = "dentists_patient"
string dataobject = "d_dentists_patient"
...
openwithparm(w_print_options,dw_dentists_patient)
