$PBExportHeader$w_dosspatselectrefa.srw
forward
global type w_dosspatselectrefa from window
end type
type sle_rech from singlelineedit within w_dosspatselectrefa
end type
type dw_liste_referents from u_dw within w_dosspatselectrefa
end type
type cb_cancel from commandbutton within w_dosspatselectrefa
end type
type cb_insert from commandbutton within w_dosspatselectrefa
end type
end forward
...
global w_dosspatselectrefa w_dosspatselectrefa

on w_dosspatselectrefa.create
...
on w_dosspatselectrefa.destroy
...
if ll_id > 0 then
	update patient set id_ref = :ll_id where patient_id = :message.doubleparm;
	commit;
end if
close(parent)
end event
