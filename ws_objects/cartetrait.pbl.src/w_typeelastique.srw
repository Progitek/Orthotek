$PBExportHeader$w_typeelastique.srw
forward
global type w_typeelastique from w_child
end type
type cb_print from commandbutton within w_typeelastique
end type
type st_title from statictext within w_typeelastique
end type
type cb_delete from commandbutton within w_typeelastique
end type
type cb_update from commandbutton within w_typeelastique
end type
type dw_typeelastique from u_dw within w_typeelastique
end type
type cb_close from commandbutton within w_typeelastique
end type
type cb_insert from commandbutton within w_typeelastique
end type
end forward
...
cb_delete cb_delete
cb_update cb_update
dw_typeelastique dw_typeelastique
cb_close cb_close
cb_insert cb_insert
end type
global w_typeelastique w_typeelastique
...
public subroutine sauve ();if dw_typeelastique.Update() = 1 then
	commit using SQLCA;
	dw_typeelastique.retrieve(v_no_ortho)
	change = false
else
...
this.dw_typeelastique=create dw_typeelastique
...
this.Control[iCurrent+5]=this.dw_typeelastique
this.Control[iCurrent+6]=this.cb_close
this.Control[iCurrent+7]=this.cb_insert
end on

on w_typeelastique.destroy
call super::destroy
destroy(this.cb_print)
destroy(this.st_title)
destroy(this.cb_delete)
destroy(this.cb_update)
destroy(this.dw_typeelastique)
...
string text = " Types d~'élastiques"
...
ll_idraptrait = dw_typeelastique.getitemnumber(dw_typeelastique.getrow(),'id_raptrait')
...
if ll_nb > 0 then
	...
	if messagebox("Suppression","Voulez-vous supprimer l'enregistrement?",Question!,YesNo!) =1 then
		dw_typeelastique.deleterow(dw_typeelastique.getrow())
		change = true
	end if
end if
end event
...
integer height = 1840
integer taborder = 10
string title = "types d~'élastique"
string dataobject = "d_typeelastique"
boolean border = false
boolean ib_isupdateable = false
...
ll_newrow = dw_typeelastique.InsertRow(0)
dw_typeelastique.ScrollToRow(ll_newrow)
dw_typeelastique.setitem(ll_newrow,'ortho_id',1)
dw_typeelastique.SetFocus()
end event
