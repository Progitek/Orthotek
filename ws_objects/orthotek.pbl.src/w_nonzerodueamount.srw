$PBExportHeader$w_nonzerodueamount.srw
forward
global type w_nonzerodueamount from w_child
end type
type cb_excel from commandbutton within w_nonzerodueamount
end type
type dw_sat from uo_sat within w_nonzerodueamount
end type
type cb_1 from commandbutton within w_nonzerodueamount
end type
type cb_close from commandbutton within w_nonzerodueamount
end type
type cb_print from commandbutton within w_nonzerodueamount
end type
type dw_nonzerodueamount from u_dw within w_nonzerodueamount
end type
end forward
...
cb_close cb_close
cb_print cb_print
dw_nonzerodueamount dw_nonzerodueamount
end type
global w_nonzerodueamount w_nonzerodueamount

on w_nonzerodueamount.create
int iCurrent
call super::create
...
this.dw_nonzerodueamount=create dw_nonzerodueamount
...
this.Control[iCurrent+6]=this.dw_nonzerodueamount
end on

on w_nonzerodueamount.destroy
call super::destroy
...
destroy(this.dw_nonzerodueamount)
end on
...
event clicked;if dw_nonzerodueamount.SaveAs("c:\ii4net\orthotek\correspondances\nonzerodueamount.xls", EXCEL5!,True) = 1 then
...
if il_idsat = 0 then
	dw_nonzerodueamount.setFilter("")
	dw_nonzerodueamount.filter()
else
	dw_nonzerodueamount.setFilter("id_satellite = " + string(il_idsat))
	dw_nonzerodueamount.filter()
end if
end event
...
gi_imp = 1
gb_datawindow = true
openwithparm(w_print_options,dw_nonzerodueamount)
...
integer height = 1836
integer taborder = 50
string title = "nonzerodueamount"
string dataobject = "d_nonzerodueamount"
...
Do While SQLCA.SQLCode = 0
   Fetch souffrance Into    :ls_patient_nom,
	...
	if SQLCA.SQLCode = 0 then
		ll_row = dw_nonzerodueamount.insertrow(0)
		dw_nonzerodueamount.scrolltorow(ll_row)
		dw_nonzerodueamount.setitem(ll_row,'patient_nom',ls_patient_nom)
		dw_nonzerodueamount.setitem(ll_row,'patient_prenom',ls_patient_prenom)
		dw_nonzerodueamount.setitem(ll_row,'dr_nom_complet',ls_nom_complet)
		dw_nonzerodueamount.setitem(ll_row,'traitement_id',ll_traitement_id)
		dw_nonzerodueamount.setitem(ll_row,'cout_traitement',montant_original)
		dw_nonzerodueamount.setitem(ll_row,'dernier_paiement',dernpaiement)
		dw_nonzerodueamount.setitem(ll_row,'dernpaie',datedernpaie)
		dw_nonzerodueamount.setitem(ll_row,'montant_paye',montant_paye)
		dw_nonzerodueamount.setitem(ll_row,'totfact',totfact)
		dw_nonzerodueamount.setitem(ll_row,'montant_facture',montant_facture)
		dw_nonzerodueamount.setitem(ll_row,'id_satellite',ll_idsat)
	 end if
Loop
...
event constructor;if v_langue = 'an' then
	this.object.t_titre.text = 'WEEKLY REPORT'
...
dw_nonzerodueamount.event retourner(v_no_ortho)
