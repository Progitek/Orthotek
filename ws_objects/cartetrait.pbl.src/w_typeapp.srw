$PBExportHeader$w_typeapp.srw
forward
global type w_typeapp from w_child
end type
type cb_print from commandbutton within w_typeapp
end type
type st_title from statictext within w_typeapp
end type
type cb_delete from commandbutton within w_typeapp
end type
type cb_update from commandbutton within w_typeapp
end type
type dw_typeapp from u_dw within w_typeapp
end type
type cb_close from commandbutton within w_typeapp
end type
type cb_insert from commandbutton within w_typeapp
end type
end forward
...
public subroutine sauve ();if dw_typeapp.Update() = 1 then
	commit using SQLCA;
	dw_typeapp.retrieve(v_no_ortho)
	change = false
else
...
dw_typeapp.setitem(ll_newrow,'ortho_id',1)
dw_typeapp.SetFocus()
end event
